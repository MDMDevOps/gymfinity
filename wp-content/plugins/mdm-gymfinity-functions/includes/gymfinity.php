...
class Gymfinity {
	...
	private function register_staff_hooks() {
		$module = \Mdm\Gymfinity\Posts\Staff::get_instance();
		add_action( 'init', array( '\\Mdm\\Gymfinity\\Posts\\Staff', 'register' ) );
		add_action( 'add_meta_boxes', array( $module, 'add_meta_boxes' ) );
		add_action( 'save_post', array( $module, 'save_metabox' ), 10, 3 );
		add_action( 'staff_position', array( $module, 'output_staff_position' ) );
	}
